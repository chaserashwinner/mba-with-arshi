...
import { ABROAD_FILTER_DEFAULTS } from "../constants/medicineAbroadConstants";
import {
  applyAbroadCollegeFilters,
  buildAbroadCollegeQuery,
...
export const useAbroadCollegeFilters = (colleges, options = {}) => {
  ...
  const forcedCourse = computed(() =>
    normalizeAbroadCourse(normalizeRouteParam(route.params.course)),
  );
  const forcedCountry = computed(() => normalizeRouteParam(route.params.country));

  const filteredColleges = computed(() => {
    const items = colleges.value || [];
    if (options.serverSide) return items;

    return applyAbroadCollegeFilters(items, filters.value);
  });
  ...
  const syncQuery = async () => {
    ...
    const nextQuery = buildAbroadCollegeQuery(filters.value, route);
  ...
  const clearFilters = () => {
    ...
    filters.value = {
      ...
      countries: forcedCountry.value ? routeFilters.countries : [],
    };
    searchInput.value = "";
    void syncQuery();
  };
  ...
  watch(
    () => [route.params.course, route.params.country, route.query],
    () => {
